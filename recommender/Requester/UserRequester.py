...
from recommender.Entity.UserEntity import UserEntity
...
class UserRequester(BaseRequester):
    ...
    def get_user_fork(self, username):
        url = f"https://github.com/{username}?tab=repositories&q=&type=fork&language=&sort="
        self._test()
        result = requests.get(url=url, headers=self._random_header()).text
        fork_repo = Parser.parser_fork(result)
        return fork_repo
    ...
    def __get_users(self, user_num) -> list:
        ...
        for i in range(page):
            ...
            url = f"https://api.github.com/users?repos>50&since={since}&per_page={page_size}"
            self._test()
            json_result = requests.get(url=url, headers=self._random_header()).json()
            user_list.extend(Parser.parser_user(json_result))
        return user_list
    ...
    def __get_user_repos(self, username, repo_num) -> list:
        ...
        repos = list()
        for i in range(page):
            url = f"https://github.com/{username}?tab=repositories&q=&type=source&language=&sort="
            self._test()
            result = requests.get(url=url, headers=self._random_header()).text
            repos = Parser.parser_user(result)
        return repos
...
if __name__ == '__main__':
    ...
    # 保存user—forkrepo
    # da = pd.read_csv('../files/user_info.csv')
    user_info = []
    # for r in da.iterrows():
    #     user_info.append(r[1][2])
    # print(user_info)
    # repos = []
    # for i in range(0, len(user_info)):
    #     fork_dict = {'login': user_info[i],
    #                  'forkrepo': str(user_requester.get_user_fork(user_info[i]))}
    #     df = pd.DataFrame(fork_dict, index=[0])
    #     df.to_csv("../files/repo_fork.csv", mode="a", header=False)
    print(user_requester.get_user_fork("zhengnaying"))
